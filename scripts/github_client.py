...
class GitHubClient:
    ...
    def find_issue_by_title_prefix(self, requirement_id: str) -> dict | None:
        ...
        followed by a non-alphanumeric separator (space, em-dash, ']', etc.).

        Anchored prefix match prevents `KPM-1.1` from matching `KPM-1.10`
        or `FR-2` from matching `FR-20`. Photo-workflow titles are
        `[<ID>] <short>` (bracketed) or `<ID> — <short>`; both start
        with the ID after an optional `[`.
        """
        r = self._session.get(
            f"{self.REST_BASE}/repos/{self.owner}/{self.repo}/issues",
            params={"state": "open", "per_page": 100},
