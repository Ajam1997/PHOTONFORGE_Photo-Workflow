...
def test_compute_alpha_decay_rate():
    ...
    assert alpha_slow > alpha_default
...
def test_load_corpus_old_format(tmp_path: Path):
    """_load_corpus should parse old flat genre format."""
...
def test_load_corpus_new_format(tmp_path: Path):
    """_load_corpus should parse new two-axis format."""
    corpus_path = tmp_path / "genres.jsonl"
    corpus_path.write_text(
        '{"filename": "A.ARW", "subject": "wildlife", "photo_type": "general", "source_folder": "TEST_1"}\n'
        '{"filename": "B.ARW", "subject": "people", "photo_type": "portrait", "source_folder": "TEST_1"}\n'
    )
    result = _load_corpus(corpus_path)
    assert result["A.ARW"] == ["wildlife"]
    assert result["B.ARW"] == ["people", "portrait"]


def test_load_corpus_new_format_general_type(tmp_path: Path):
    """_load_corpus should omit photo_type='general' from labels list."""
    corpus_path = tmp_path / "genres.jsonl"
    corpus_path.write_text(
        '{"filename": "A.ARW", "subject": "cat", "photo_type": "general", "source_folder": "TEST_1"}\n'
    )
    result = _load_corpus(corpus_path)
    assert result["A.ARW"] == ["cat"]
...
def test_load_embeddings_from_photon_db(tmp_path: Path):
    ...
    conn.execute("INSERT INTO TEST_1 VALUES (?, ?)", ("B.ARW", None))
    conn.commit()
    conn.close()
...
def test_compute_centroids_single_label(tmp_path: Path):
    """compute_centroids should group embeddings by label."""
...
def test_compute_centroids_multi_label(tmp_path: Path):
    """compute_centroids trains every label in the list."""
...
def _expected_prototype_rows() -> int:
    return len(SUBJECTS) + len(PHOTO_TYPES)
...
def test_recalibrate_prototypes_small_n(tmp_path: Path):
    ...
    n_rows = _expected_prototype_rows()
    hardcoded = np.random.randn(n_rows, 512).astype(np.float32)
...
def test_recalibrate_prototypes_large_n(tmp_path: Path):
    ...
    for i in range(15):
        ...
        lines.append(
            f'{{"filename": "{filename}", "subject": "wildlife", "photo_type": "general", "source_folder": "TEST_1"}}'
        )
    corpus_path.write_text("\n".join(lines))
...
def test_recalibrate_prototypes_cosine_similarity(tmp_path: Path):
    ...
    n_rows = _expected_prototype_rows()
    hardcoded = np.random.randn(n_rows, 512).astype(np.float32)
    # Put learned centroid at wildlife's position in subject half
    wildlife_idx = SUBJECTS.index("wildlife")
    hardcoded[wildlife_idx] = learned_centroid.copy()
    for i in range(n_rows):
        hardcoded[i] /= np.linalg.norm(hardcoded[i])
    ...
    blended = result["wildlife"]["prototype"]
    cosine_sim = np.dot(blended, hardcoded[wildlife_idx])
