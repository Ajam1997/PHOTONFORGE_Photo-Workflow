# Bootstrap Prompt — Photo-Workflow (RETROSPECTIVE INCEPTION RECORD)

> **NOTE — RETROSPECTIVE.** This is a backfilled inception record
> authored 2026-05-29 during the migration from photo-workflow's
> ad-hoc structure to the systems-first-template format
> (see `dev-docs/migration-plan.md` Phase 2). The project actually
> started earlier in 2026; the answers below describe its state at
> the time of migration, not at original conception. Future bootstrap
> records on other projects will be written contemporaneous with the
> start of work.
...
> **Project:** `<<An offline photography pipeline for a hobbyist photographer that ingests RAW files from an SD card on a Lenovo Yoga 910-13IKB, scores them for sharpness/composition/exposure, names them via Florence-2 INT8 ONNX captioning, and syncs the results into Darktable — all without network access at runtime.>>`
...
> **If custom, name the closest base profile:** `<<n/a>>`

### A3. Top-level user-need count target

Aim for 6–12 on this first pass. You can add more later.

> **Target UN count:** `<<8 — to be confirmed in Phase 3 of the migration plan after classifying the 67 existing Issues>>`
...
> **Markets / regimes:** `<<Hobby / personal use only. No certification or compliance scope. The system runs entirely offline on the operator's own laptop and writes to the operator's own storage.>>`

### A5. GitHub repo slug

`owner/name`. Verify with `gh repo view <slug>` before pasting.

> **Repo slug:** `<<Ajam1997/PHOTONFORGE_Photo-Workflow>>`

### A6. Operator + date  *(inception provenance)*

> **Operator:** `<<Ajam1997>>`
> **Date filled:** `<<2026-05-29 (retrospective; project actually started earlier in 2026)>>`
