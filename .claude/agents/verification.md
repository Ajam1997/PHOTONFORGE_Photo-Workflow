...
every remote command:
  ...
  ssh alex@<yoga-ip> 'cd ~/PHOTONFORGE_Photo-Workflow && source .venv/bin/activate && [command]'
...
Retrieve diff from the Yoga 910:
  ssh alex@<yoga-ip> 'cd ~/PHOTONFORGE_Photo-Workflow && git diff HEAD~1 HEAD -- src/ models/'
...
Run only test files corresponding to changed modules:
  ssh alex@<yoga-ip> 'cd ~/PHOTONFORGE_Photo-Workflow && source .venv/bin/activate && pytest tests/test_[module].py -v --tb=short'
...
Triggered by: naming.py or models/ in diff
  ...
  ssh alex@<yoga-ip> 'cd ~/PHOTONFORGE_Photo-Workflow && source .venv/bin/activate && python3 -c "
import time
...
Triggered by: pipeline.py in diff
  ...
  ssh alex@<yoga-ip> '/usr/bin/time -v sh -c "cd ~/PHOTONFORGE_Photo-Workflow && source .venv/bin/activate && python -m photo_workflow.pipeline --model-dir models/florence2_int8 --input tests/fixtures/" 2>&1 | grep "Maximum resident"'
...
Check first:
  ssh alex@<yoga-ip> 'cd ~/PHOTONFORGE_Photo-Workflow && grep -r "[function_name]" tests/'

If no test exists, generate and write:
  ssh alex@<yoga-ip> 'cat >> ~/PHOTONFORGE_Photo-Workflow/tests/test_[module].py << EOF
...
Run the new test immediately after writing:
  ssh alex@<yoga-ip> 'cd ~/PHOTONFORGE_Photo-Workflow && source .venv/bin/activate && pytest tests/test_[module].py::test_[function] -v --tb=short'
