...
local DEFS = {
  ...
  { name = "models_path",      type = "string",  default = "", label = "Models dir (blank = auto: <drive>/models on a portable cartridge, else the CLI auto-detects from its package)" },
  { name = "cli_path",         type = "string",  default = "", label = "photo-workflow CLI path (blank = auto: <drive>/runtime/ on a portable cartridge, else 'photo-workflow' on PATH; set to <venv>/Scripts/photo-workflow.exe when Darktable can't see the venv)" },
  { name = "cartridge_path",   type = "string",  default = "", label = "photo-cartridge program path -- ADVANCED, leave blank (auto: derived from the photo-workflow path, or <drive>/runtime/). This is the .exe, NOT your cartridge drive" },
