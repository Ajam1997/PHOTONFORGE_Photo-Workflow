...
> **v2.0 taxonomy change:** Subjects revised 16 → 13, Photo Types revised 12 → 11, and the two axes are now **fully orthogonal** (no label appears on both).
...
| Milky Way over a mountain landscape | S-07 Sky (sky is the hero) OR S-05 Landscape (if land is equally important) | T-10 Long Exposure | Astro lives in `sky` now; the long exposure technique is the defining Type. |
...
| Intentionally blurred city lights | S-13 Abstract | T-10 Long Exposure | Only use Abstract when the result is genuinely non-representational. |
...
6. **Aerial and Long Exposure override other types.** A drone shot of architecture is T-09 Aerial; a long-exposure waterfall in a landscape is T-10 Long Exposure.
