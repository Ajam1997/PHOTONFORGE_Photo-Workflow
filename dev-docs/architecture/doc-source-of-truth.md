...
|---|---|---|
| @verification | Issue comments (with `Next action:` and `via:` footers) via `github_comment.py` | `dev-docs/living-user-needs.md` (any section); status labels |
| @validation | Issue comments via `github_comment.py`; `tests/e2e/*` | `dev-docs/living-user-needs.md`; status labels; `src/`; `tests/test_*` |
