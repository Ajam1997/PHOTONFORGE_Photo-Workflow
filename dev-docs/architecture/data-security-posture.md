...
| **No backup of the catalog** | If the SSD dies, `photonforge.db` is gone. A disaster-recovery gap, not an exposure one. | **Mitigated** — `src/photo_workflow/backup.py`, two tiers: `photo-cartridge snapshot` (catalog copy on the cartridge, seconds, no dependencies) and `photo-cartridge backup --dest` (verified whole-cartridge mirror to a second drive, checksummed, with `verify-backup` / `restore-backup`). Both are stdlib-only and offline. Panel buttons in the CARTRIDGE strip; optional snapshot-on-eject via `PHOTONFORGE_SNAPSHOT_ON_EJECT=1`. See [ADR-007](adr/ADR-007-cartridge-backup-tiers.md) and the [backup runbook](../backup-and-restore-guide.md). **Residual:** backups are local-only — an encrypted off-site tier (restic) is designed but not built, so a fire or theft taking both drives still loses the catalog. Unencrypted at rest, consistent with the row below. |
